...
func Serve(transport string) *server.MCPServer {
	...
	if transport == "http" {
		...
		// Railway injects PORT; fall back to Render's default when unset.
		addr := ":10000"
...
func newHTTPMux(mcpHandler http.Handler, oauthCfg oauth.Config, openAIToken string) *http.ServeMux {
	...
	// authn.SecretMiddleware is the outermost gate (X-Webhook-Secret / MCP_SECRET).
	// It runs before OAuth so unauthorized callers never reach the MCP handler.
	mux.Handle("/mcp", authn.SecretMiddleware(oauthMiddleware(mcpHandler)))
